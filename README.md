...
the deadline?"*, *"which photos in the site group got a ⭐ reaction?"*

- **Pure Node, no native builds.** Uses Node 24's built-in SQLite. No Go, no Python, no compiler. Works on Windows.
- **Read-only by design.** The MCP server exposes eight read tools and nothing else.
...
All optional, set as environment variables for `bridge.js`:
...
Video and voice notes are stored as metadata only (no download). Images, documents and stickers up to
`WA_MAX_MEDIA_MB` are downloaded for live messages; history sync records metadata without downloading.
...
| `media/` | Downloaded attachments | Plain files |
...
- **Nothing leaves your machine on its own.** The bridge only connects outbound to WhatsApp. The MCP server
...
- **Wipe:** delete `auth/`, `store.db*`, `media/`.
